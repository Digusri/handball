...
import { Team, Match, NewsArticle, User, UserRole, Player } from './types';
...
import { Calendar, Trophy, Newspaper, TrendingUp, MapPin, Clock, Search, Shield, Plus, Trash2, Wand2, Users, Medal, User as UserIcon, Upload, FileText, CheckCircle, Loader2, ArrowLeft, Save, Pencil, X, Camera, Lock, Key, Sparkles } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  // Data State with Persistence (Initialize from LocalStorage if available)
  const [teams, setTeams] = useState<Team[]>(() => {
    const saved = localStorage.getItem('hc_teams');
    return saved ? JSON.parse(saved) : MOCK_TEAMS;
  });
  const [matches, setMatches] = useState<Match[]>(() => {
    const saved = localStorage.getItem('hc_matches');
    return saved ? JSON.parse(saved) : MOCK_MATCHES;
  });
  const [news, setNews] = useState<NewsArticle[]>(() => {
    const saved = localStorage.getItem('hc_news');
    return saved ? JSON.parse(saved) : MOCK_NEWS;
  });
  const [players, setPlayers] = useState<Player[]>(() => {
    const saved = localStorage.getItem('hc_players');
  ...
  // Initial Load User Session
  useEffect(() => {
    const storedUser = localStorage.getItem('hc_active_user');
    if (storedUser) {
  ...
  // --- PERSISTENCE EFFECTS ---
  useEffect(() => localStorage.setItem('hc_teams', JSON.stringify(teams)), [teams]);
  useEffect(() => localStorage.setItem('hc_matches', JSON.stringify(matches)), [matches]);
  useEffect(() => localStorage.setItem('hc_news', JSON.stringify(news)), [news]);
  useEffect(() => localStorage.setItem('hc_players', JSON.stringify(players)), [players]);
  ...
  const updateTeamStats = (updatedMatches: Match[]) => {
    const newTeams = teams.map(team => {
      ...
      teamMatches.forEach(match => {
        ...
        goalDifference += (myScore - oppScore);
        ...
        if (myScore > oppScore) {
          ...
          lost++;
          // 1 point for losing in some rules, but standard is 0 or 1 depending on federation. Assuming 0 for now unless defined.
          // Let's stick to standard 3-1-0.
          points += 0; 
        } else {
          drawn++;
          points += 1;
      ...
      return { ...team, points, played, won, lost, drawn, goalDifference };
    });
    setTeams(newTeams);
  };
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    const foundUser = users.find(u => u.email.toLowerCase() === loginEmail.toLowerCase() && u.password === loginPass);
    if (foundUser) {
      setUser(foundUser);
      localStorage.setItem('hc_active_user', JSON.stringify(foundUser));
      setCurrentView('home');
      setLoginPass('');
    } else {
  ...
  const handleLogout = () => {
    setUser(null);
    localStorage.removeItem('hc_active_user');
    setCurrentView('home');
  };
  ...
  const handleCreateUser = (e: React.FormEvent) => {
    ...
    setNewUser({ name: '', email: '', password: '', role: UserRole.ADMIN });
  ...
  const handleGenerateNews = async () => {
    ...
    try {
      ...
      const newArticle: NewsArticle = {
        ...
        imageUrl: `https://images.unsplash.com/photo-1541252260730-0412e8e2108e?w=800&fit=crop`,
  ...
  const handleApplyAnalysis = () => {
    ...
    // 1. Update Match Results (Need to select which match this belongs to, for demo we create a new one or need to link it)
    // For this demo, let's auto-create a finished match with the detected teams
    const homeTeam = teams.find(t => t.name.toLowerCase().includes(analysisResult.homeTeam?.toLowerCase()) || analysisResult.homeTeam?.toLowerCase().includes(t.name.toLowerCase()));
    const awayTeam = teams.find(t => t.name.toLowerCase().includes(analysisResult.awayTeam?.toLowerCase()) || analysisResult.awayTeam?.toLowerCase().includes(t.name.toLowerCase()));
    ...
    if (homeTeam && awayTeam) {
       const newMatch: Match = {
         ...
         federationId: homeTeam.federationId,
         category: homeTeam.category,
         location: 'Cargado por Planilla IA',
         status: 'FINISHED'
       };
       
       const newMatches = [...matches, newMatch];
       setMatches(newMatches);
       updateTeamStats(newMatches); // Update standings immediately
    } else {
       alert("No se encontraron los equipos automáticamente. Se actualizarán solo los goleadores.");
    }

    // 2. Update Players Goals
    let updatedPlayers = [...players];
    let playersUpdatedCount = 0;

    if (analysisResult.players) {
        analysisResult.players.forEach((p: any) => {
            // Fuzzy match logic simplified
            const playerIndex = updatedPlayers.findIndex(
                existing => existing.surname.toLowerCase().includes(p.name.toLowerCase()) || 
            ...
            if (playerIndex !== -1) {
                updatedPlayers[playerIndex] = {
                    ...updatedPlayers[playerIndex],
                    goals: updatedPlayers[playerIndex].goals + (p.goals || 0)
                };
                playersUpdatedCount++;
            }
        });
    ...
    alert(`Planilla procesada.\n\nResultado registrado: ${analysisResult.homeScore} - ${analysisResult.awayScore}\nJugadores actualizados: ${playersUpdatedCount}`);
    setAnalysisResult(null);
  };
  ...
  const handleSaveMatch = (e: React.FormEvent) => {
    ...
    let newMatchesList = [...matches];

    if (matchForm.id) {
      newMatchesList = matches.map(m => m.id === matchForm.id ? { ...m, ...matchForm } as Match : m);
    } else {
      const newMatch: Match = {
        ...(matchForm as Match),
        id: Date.now().toString(),
    ...
    if (matchForm.status === 'FINISHED') {
        updateTeamStats(newMatchesList);
    }
  ...
  const handleDeleteMatch = (matchId: string) => {
    if (confirm("¿Seguro de eliminar este partido?")) {
      ...
      updateTeamStats(updatedMatches); // Re-calc standings
    }
  };
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
           <div className="flex justify-between items-center mb-4">
             <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
               <Shield className="text-blue-600" /> Gestión de Usuarios (Admins)
             </h2>
             <button onClick={() => setIsUserModalOpen(true)} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg text-sm flex items-center gap-2">
               ...
               <tbody>
                 {users.map(u => (
                   <tr key={u.id} className="border-b border-slate-50 last:border-0">
                     ...
                     <td className="px-4 py-3"><span className="bg-blue-100 text-blue-700 px-2 py-0.5 rounded text-xs font-bold">{u.role}</span></td>
                   </tr>
                 ))}
               </tbody>
             </table>
        ...
        <div className="relative z-10 flex flex-col md:flex-row gap-8">
          <div className="flex-1">
             ...
             <p className="text-blue-200 mb-6">
               Sube una foto o <strong>PDF</strong> de la planilla oficial. La IA analizará la imagen, <strong>actualizará automáticamente el resultado del partido y sumará los goles</strong> a cada jugador.
             </p>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            ...
            <form onSubmit={handleCreateUser} className="p-6 space-y-4">
              <div>
                ...
                <input required type="text" value={newUser.name} onChange={e => setNewUser({...newUser, name: e.target.value})} className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Email</label>
                <input required type="email" value={newUser.email} onChange={e => setNewUser({...newUser, email: e.target.value})} className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Contraseña</label>
                <input required type="password" value={newUser.password} onChange={e => setNewUser({...newUser, password: e.target.value})} className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Rol</label>
                <select value={newUser.role} onChange={e => setNewUser({...newUser, role: e.target.value as UserRole})} className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
                  <option value={UserRole.ADMIN}>Administrador</option>
                  <option value={UserRole.USER}>Usuario Normal</option>
                </select>
              </div>
              <div className="pt-4">
                <button type="submit" className="w-full bg-blue-600 text-white font-bold py-3 rounded-lg hover:bg-blue-700 transition-colors">Crear Usuario</button>
              </div>
            </form>
          </div>
        </div>
